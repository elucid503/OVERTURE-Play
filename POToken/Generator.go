...
// BgUtilGenerator automatically generates PO tokens using the bgutil HTTP server
// This requires running the bgutil-ytdlp-pot-provider server:
//
//	Docker:  docker run --name bgutil-provider -d -p 4416:4416 brainicism/bgutil-ytdlp-pot-provider
//	Native:  See https://github.com/Brainicism/bgutil-ytdlp-pot-provider for setup
//
// The server handles all BotGuard attestation complexity internally.
//
// IMPORTANT: Session Binding
// ==========================
// PO tokens are bound to the BotGuard session that generated them. The bgutil server
// generates tokens using its OWN internal BotGuard session, not any browser/cookie session.
//
// This means:
// - Tokens generated by bgutil will work for UNAUTHENTICATED requests only
// - If you use a cookie (logged-in), the token won't match the cookie's session → 403 errors
// - For server-side applications, DON'T use cookies - let bgutil handle the session
// - The visitor_data from API responses can be used as content binding for the token
