...
const actionsObjRegex = new RegExp(`var (${jsVarStr})=\\{((?:(?:${jsKeyStr}${reverseStr}|${jsKeyStr}${sliceStr}|${jsKeyStr}${spliceStr}|${jsKeyStr}${swapStr}),?\\r?\\n?)+)\\};`);
const actionsFuncRegex = new RegExp(`function(?: ${jsVarStr})?\\(a\\)\\{a=a\\.split\\((?:''|"")\\);\\s*((?:(?:a=)?${jsVarStr}${jsPropStr}\\(a,\\d+\\);)+)return a\\.join\\((?:''|"")\\)\\}`);
...
export class Decipherer {
    ...
    sigTokens;
    nSolver;
    constructor(playerCode, playerURL = "") {
    ...
    extractSignatureTokens() {
        ...
        const tokenizeRegex = new RegExp(`(?:a=)?${obj}(?:\\.${keys}|\\[(?:'${keys}'|"${keys}")\\])\\(a,(\\d+)\\)`, "g");
        let match;
...
export function GetSignatureTimestamp(playerCode) {
...
export function ExtractPlayerID(playerURL) {
...
export async function NewDecipherer(playerCode, playerURL = "") {
