...
import { Handler } from "./stream/handler.js";
async function main() {
    ...
    for (const format of video.Formats.filter((f) => f.Width > 0 && f.Height > 0)) {
        console.log(`  ITag: ${format.ITag.toString().padEnd(4)} | ${(format.QualityLabel || format.Quality).padEnd(10)} | ${format.Width}x${format.Height} @ ${format.FPS}fps | ${Extension(format)}`);
    }
    console.log("\nAudio Formats:");
    const audioFormats = AudioFormats(video);
    for (const format of audioFormats) {
        console.log(`  ITag: ${format.ITag.toString().padEnd(4)} | ${(format.AudioQuality || "unknown").padEnd(15)} | ${Math.floor(format.Bitrate / 1000)} kbps | ${Extension(format)}`);
    }
    ...
    try {
        const data = await streamHandler.DownloadWithProgress(bestAudio, (p) => {
            if (p.Total > 0) {
                ...
                process.stdout.write(`\r  Progress: ${percent.toFixed(1)}% (${mbDownloaded.toFixed(2)} / ${mbTotal.toFixed(2)} MB) @ ${speed.toFixed(2)} MB/s`);
            }
            else {
                const mbDownloaded = p.Downloaded / 1024 / 1024;
