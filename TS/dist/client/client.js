...
import { CreateEmptyVideo } from "../types/video.js";
import { CreateEmptyFormat } from "../types/format.js";
import { GetContext, GetContextWithVisitor } from "../innertube/config.js";
...
import { NewDecipherer, ExtractPlayerID } from "../decipher/decipher.js";
import { Auth, ExtractVisitorDataFromHTML } from "../auth/auth.js";
...
export class Client {
    ...
    async fetchPlayerCode(playerURL) {
    ...
    async fetchWithClient(videoID, clientConfig) {
    ...
    async getPlayerPOToken(videoID, clientConfig) {
    ...
    async getGVSPOToken(videoID, clientConfig) {
    ...
    requiresPoToken(clientConfig) {
        for (const protocol of Object.keys(clientConfig.GVSPoTokenPolicies)) {
            const policy = clientConfig.GVSPoTokenPolicies[protocol];
    ...
    async parsePlayerResponse(data, clientConfig, videoID) {
        let resp;
        try {
        ...
        if (resp.playabilityStatus.status !== "OK") {
            throw new Error(`video not playable: ${resp.playabilityStatus.status} - ${resp.playabilityStatus.reason || "unknown reason"}`);
        }
    ...
    addPOTokenToURL(streamURL, poToken) {
        try {
    ...
    getAPIRequestHeaders(clientConfig) {
        const headers = {
