...
export class Auth {
    ...
    static FromJSON(path: string): Auth {
        ...
        const jsonCookies: JSONCookie[] = JSON.parse(content);
        const cookies: Cookie[] = [];
...
export function ExtractVisitorDataFromHTML(html: string): string {
    const patterns = [
        /"VISITOR_DATA"\s*:\s*"([^"]+)"/,
...
export function ExtractDataSyncIDFromResponse(data: string): string {
